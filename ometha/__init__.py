...
import urllib3
from harvester import SEP_LINE, ACHTUNG, FEHLER, INFO, __version__
...
╚██████╔╝██║ ╚═╝ ██║███████╗   ██║   ██║  ██║██║  ██║ 
 ╚═════╝ ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝
    ...
    config_path = os.path.join(os.path.expanduser("~"), ".ometha.yaml") if sys.platform.startswith(
        ("linux", "darwin")) else os.path.join(application_path, "ometha.yaml")
    if os.path.exists(config_path):
        ...
        if input("Konfigurationsdatei mit abweichenden Werten für den http-Header erstellen? y/N: ").lower() == "y":
            ...
            with open(os.path.join(config_path, "ometha.yaml"), "w", encoding="utf8") as f:
    ...
    adapter = HTTPAdapter(max_retries=urllib3.util.retry.Retry(total=4, status_forcelist=[429, 500, 502, 503, 504], backoff_factor=2))
    ...
    logger.add(log_file, level=0,
               format="<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: <8}</level> | <level>{message}</level>",
               enqueue=True)
    parameters = {"Ometha Version": __version__, "Mode": f"{running_mode} with {PRM['mode']} Mode", "Datengeber": PRM["dat_geb"],
                  "Base-URL": PRM["b_url"], "metadataPrefix": PRM["pref"], "Sets": PRM["sets"], "Timeout": PRM["timeout"],
                  "Outputfolder": PRM["out_f"], "Count of parallel downloads": PRM["n_procs"]}
    ...
    try:
        ...
        print_and_exit(f"{FEHLER}{e}", PRM["mode"])
    ...
    if PRM["sets"] is not None:
        ...
        a_sets = PRM.get("sets", {}).get("additive", [])
        i_sets = PRM.get("sets", {}).get("intersection", [])
        # Initialize lists for comma and slash ids
        a_ids = [id for a_set in a_sets for id in generate_id_harvesting_url(PRM, a_set)]
        i_ids = [id for i_set in i_sets for id in generate_id_harvesting_url(PRM, i_set)]
    ...
    if failed_ids or failed_download:
        ...
        print_and_log(f"{SEP_LINE}{ACHTUNG} Es wurden IDs aufgrund von Verbindungsproblemen übersprungen.\n  Siehe dazu {failed_idfile}{SEP_LINE}{INFO} fehlgeschlagene IDs erneut harvesten..", logger, "warning")
