...
def interactiveMode(session):
    ...
    if how_to_start == "I":
        ...
        PRM["id_f"] = get_valid_input(
            "idfile: ",
            lambda file: os.path.exists(file),
            "Bitte geben Sie eine valide ID-Datei an.",
        )
        try:
        ...
        PRM["res_tok"] = get_valid_input(
            "Resumption Token: ",
            lambda token: bool(token),
            "Resumption Token darf nicht leer sein.",
        )
...
def get_sets_mprefs(url, session, sets: dict):
    ...
    for s in root.findall(f".//{NAMESPACE}set"):
        name, spec = (
            s.findtext(f".//{NAMESPACE}setName"),
            s.findtext(f".//{NAMESPACE}setSpec"),
        )
        if name in sets:
